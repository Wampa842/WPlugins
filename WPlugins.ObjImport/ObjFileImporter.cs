...
namespace WPlugins.ObjImport
{
    public class ObjFileImporter
	{
		...
		private Dictionary<Tuple<int, int, int>, int> vertexDict;					//Assigns indices to v/vt/vn hashes
        ...
        private void Worker_ProcessFile(object sender, DoWorkEventArgs e)
        {
        ...
        //public ObjFileImporter(string path, IPXPmxBuilder builder, Settings settings, bool sumthin)
        //{
        //    this.builder = builder;
        //    this.settings = settings;
        //    this.ObjFileName = Path.GetFileNameWithoutExtension(path);
        ...
        public IPXPmx ToPmx()
		{
			...
			foreach (IPXVertex v in vertices)
			{
				...
				if (settings.CreateBone == Settings.CreateBoneMode.Average)
					bone.Position += v.Position;
				//Weight vertices to the new bone if needed
				if (settings.CreateBone != Settings.CreateBoneMode.None)
					v.Bone1 = bone;
				else
					v.Bone1 = pmx.Bone[0];
				v.Weight1 = 1.0f;
			...
			foreach (var m in materials)
			{
				...
				if (settings.MaterialNaming == Settings.MaterialNamingMode.BitmapName)
					m.Name = m.NameE = m.Tex;
				if (settings.MaterialNaming == Settings.MaterialNamingMode.Numbered)
