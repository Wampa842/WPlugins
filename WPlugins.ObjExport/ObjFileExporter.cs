...
namespace WPlugins.ObjExport
{
    public class ObjFileExporter
	{
		...
		private ImportProgressForm progressForm;    //UI to report progress and cancel the background process

		public ObjFileExporter(string path, IPXPmx pmx, Settings settings)
		{
		...
		private void Worker_ProcessPmx(object sender, DoWorkEventArgs e)
		{
			...
			foreach (IPXMaterial m in pmx.Material)
			{
				...
				if (!string.IsNullOrWhiteSpace(m.Tex))
				{
					...
					{
						case Settings.BitmapActionType.Copy:
							...
							break;
						case Settings.BitmapActionType.Link:
							textureNewPath = Path.Combine(settings.BitmapPath, m.Tex);
							break;
						case Settings.BitmapActionType.Absolute:
		...
		public void ProcessPmx()
		{
			...
			progressForm = new ImportProgressForm(worker, faceCount);
		...
		public void SaveFiles()
		{
			...
			try
			{
				...
				//Write file info
				writer.WriteLine($"# WPlugins OBJ Exporter v{Common.Info.Version} - copyright (C) 2018 Wampa842 (GNU GPL-3.0-or-later)");
